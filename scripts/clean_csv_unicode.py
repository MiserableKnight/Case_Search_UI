...
用于清理CSV文件中的Unicode控制字符，特别是从右到左标记(U+200F)等字符。
...
def batch_clean_csv_files(
    ...
    for file_path in files:
        try:
            output_file = output_dir / f"{file_path.stem}_cleaned{file_path.suffix}"
            result = clean_csv_file(str(file_path), str(output_file), encoding)
...
def main():
    parser = argparse.ArgumentParser(description='清理CSV文件中的Unicode控制字符')
    parser.add_argument('input', help='输入CSV文件路径或文件模式')
    ...
    try:
        if args.batch:
            batch_clean_csv_files(args.input, args.output_dir, args.encoding)
        else:
            clean_csv_file(args.input, args.output, args.encoding, args.output_encoding)
