...
- **前端**: Vue 3 + Element Plus + Pinia + Vite（搜索主页，`src/` 源码，构建产物在 `app/static/dist/`）；数据导入/数据分析页仍为 Vue 2.6 + Element UI（CDN 引入）
- **数据存储**: Parquet 文件

> 注：前端正处于 Vue 3 迁移中。搜索主页已迁移到 Vue 3 + Vite 的 Notion 风格新界面；旧版主页仍可通过 `/legacy` 访问。修改前端源码后需执行 `npm run build` 重新构建（开发时可用 `npm run dev`，Vite 开发服务器会把 `/api` 代理到 Flask）。
...
- 应用默认绑定 `127.0.0.1` 且**无认证机制**，请勿将其暴露到局域网或公网。
- API 的 CORS 仅允许本机来源（`127.0.0.1` / `localhost` 的应用端口，以及 Vite 开发端口 5173），其他来源的跨域请求会被浏览器拦截。
- 已知风险与完整审计记录见 `docs/adversarial_review_20260718.md`。
