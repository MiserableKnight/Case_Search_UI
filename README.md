...
- **前端**: Vue 2.6 + Element UI 2.15 + Axios（CDN 引入，无构建步骤）
- **数据存储**: Parquet 文件

> 注：根目录的 `package.json`、Vite、Element Plus 依赖是一套未完成的 Vue 3 迁移代码，当前没有任何页面引用它们；实际运行的前端以 `app/templates/` 和 `app/static/js/` 为准。
