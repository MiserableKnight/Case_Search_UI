...
from app.core.error_handler import ValidationError, ServiceError, InternalError
...
class SimilarityService:
    ...
    def get_available_methods(self):
        ...
        """
        try:
            return TextSimilarityCalculator.get_available_methods()
        except Exception as e:
    ...
    def preprocess_text(self, text):
        ...
        try:
            return TextSimilarityCalculator.preprocess_text(text)
        except Exception as e:
            logger.error(f"预处理文本时出错: {str(e)}")
            raise ServiceError(f"预处理文本失败: {str(e)}")
            
    def search_by_similarity(self, search_text, data_source, columns, limit=10):
        """
        根据相似度搜索数据
