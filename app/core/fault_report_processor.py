...
class FaultReportProcessor:
    ...
    def __init__(self, file_path=None):
        ...
        if not self._initialized:
            self.__initialize()
            FaultReportProcessor._initialized = True
    ...
    def __initialize(self):
        ...
        # 必需的原始列（用于验证导入的数据）
        self.REQUIRED_COLUMNS = ['运营人', '机型', '系列', '飞机序列号', '机号', '日期', '航班号', '故障报告类型', '故障ATA',
                              '维修ATA', '故障确认', '确认人', '确认时间', '问题描述', '排故措施', '飞行阶段', '发生地',
                              '是否ETOPS航班', '故障报告来源编码', '其他排故措施编码', '主要排故措施编码']
        
        # 最终需要保留的列
        self.FINAL_COLUMNS = ['日期', '维修ATA', '问题描述', '排故措施', '运营人', '机型', '飞机序列号', '机号', '数据类型',
                            '拆卸部件件号', '拆卸部件序列号', '装上部件件号', '装上部件序列号']
    ...
    def clean_data(self, df):
        ...
        def convert_date(date_str):
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"日期转换错误: {str(e)}, 值: {date_str}")
                    return pd.NaT
        ...
        if '日期' in cleaned_df.columns:
            ...
            cleaned_df['日期'] = cleaned_df['日期'].apply(convert_date)
        ...
        # 确保拆卸和装上部件列存在
        part_columns = ['拆卸部件件号', '拆卸部件序列号', '装上部件件号', '装上部件序列号']
    ...
    def analyze_changes(self):
        ...
        try:
            # 验证文件名
            self.validate_file_name(self.file_path)
    ...
    def validate_file_name(self, file_path):
        ...
        if not re.match(pattern, filename):
            ...
            raise ValueError(f"文件名格式不正确，应为'故障报告_YYYYMMDD.xlsx'，其中YYYYMMDD为日期格式（如20250312），但得到的是'{filename}'")
